...
public class GridPanel extends JPanel implements ApplicationEventSource {
	...
	public void setFinishCell(myPoint p, int agentNumber) {
		if (this._finishes[agentNumber - 1] == null) {
			if (!this.get_FinishList().contains(p)
				...
				this._grid[p.getX()][p.getY()].addTileStatus(Status.Finish,
						agentNumber);
			}
		} else {
			if (!this.get_FinishList().contains(p)
					&& (!this.get_blockList().contains(p))) {
				this._grid[this._finishes[agentNumber - 1].getX()][this._finishes[agentNumber - 1]
						.getY()].removeTileStatus(Status.Finish);
				this._finishes[agentNumber - 1] = p;
				this._grid[p.getX()][p.getY()].addTileStatus(Status.Finish,
						agentNumber);
			}
		...
		if (_finalPaths != null && !_finalPaths.isEmpty()) {
			for (Vector<myPoint> tAgentPath : _finalPaths) {
				...
				for (myPoint p : tAgentPath) {
					// int size =
					// this._grid[p.getX()][p.getY()].getTileStatuses().size();
					this._grid[p.getX()][p.getY()]
							.removeTileStatus(Status.Path);
					/*
					 * if
					 * (this._grid[p.getX()][p.getY()].getTileStatusAt(size-1)
					 * .equals(Status.Path)){
					 * this._grid[p.getX()][p.getY()].removeTileStatus
					 * (Status.Path); }
					 */
				}
			}
	...
	public void clearOpenList() {
		for (int i = 0; i < this._height; i++) {
			for (int j = 0; j < this._width; j++) {
				...
				/*
				 * for(int k=0; k < this._grid[i][j].getTileStatuses().size();
				 * k++){
				 * if(this._grid[i][j].getTileStatusAt(k)==Status.inOpenList ||
				 * this._grid[i][j].getTileStatusAt(k)==Status.inClosedList){
				 * 
				 * } }
				 */
			}
		...
		if (finalPath != null && !finalPath.isEmpty()) {
			for (int i = 0; i < finalPath.size(); i++) {
				...
				for (int j = 0; j < tPath.size(); j++) {
					...
					Direction dir = calcDirection(prePoint, currentPoint,
							nextPoint);
					drawFinalPathCell(currentPoint.getX(), currentPoint.getY(),
							i + 1, dir);
				}
			}
		}
	...
	public void drawOneFinalStepWithLines(Vector<myPoint> tStep,
			boolean withLines) {
		for (int j = 0; j < tStep.size(); j++) {
			myPoint p = tStep.elementAt(j);
			if (withLines) {
	...
	private Direction calcDirection(myPoint p, myPoint q, myPoint m) {
		...
		if (preDeltaX == 0 && preDeltaY == 1 && postDeltaX == 0
				&& postDeltaY == -1)
			res = Direction.LEFT_RIGHT;
		if (preDeltaX == 0 && preDeltaY == -1 && postDeltaX == 0
				&& postDeltaY == 1)
			res = Direction.RIGHT_LEFT;
		if (preDeltaX == -1 && preDeltaY == 0 && postDeltaX == 1
				&& postDeltaY == 0)
			res = Direction.DOWN_TOP;
		if (preDeltaX == 1 && preDeltaY == 0 && postDeltaX == -1
				&& postDeltaY == 0)
			res = Direction.TOP_DOWN;
		if (preDeltaX == 1 && preDeltaY == 1 && postDeltaX == -1
				&& postDeltaY == -1)
		...
		if (preDeltaX == 0 && preDeltaY == 1 && postDeltaX == 1
				&& postDeltaY == 0)
			res = Direction.LEFT_TOP;
		if (preDeltaX == 1 && preDeltaY == 0 && postDeltaX == 0
				&& postDeltaY == 1)
		...
		if (preDeltaX == 0 && preDeltaY == 0 && postDeltaX == 1
				&& postDeltaY == 0)
		...
		if (preDeltaX == 0 && preDeltaY == 0 && postDeltaX == 0
				&& postDeltaY == -1)
		...
		if (preDeltaX == 1 && preDeltaY == -1 && postDeltaX == 0
				&& postDeltaY == 0)
	...
	public void drawOneStep(Vector<myPoint> tStep,
			Vector<Integer> currentAgents) {
		for (int i = 0; i < tStep.size(); i++){
			myPoint p = tStep.elementAt(i);
			drawOpenListCell(p.getX(), p.getY(), currentAgents.elementAt(i) + 1);
		}
	...
	/**
	 * stops the animation once the stop button was pressed or a path has been
	 * found
	 */
	...
	/**
	 * sets the grid according to editable false - the grid cannot be edit. true
	 * - the grid can be edit.
	 * 
	 * @param editable
	 */
